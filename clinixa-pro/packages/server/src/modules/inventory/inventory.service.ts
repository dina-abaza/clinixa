...
/**
 * @description شكل الصنف في المخزون بعد إضافة الحقول المحسوبة لحظياً
 */
 ...
 */
export async function getInventoryItems(branchId?: string | null): Promise<{ items: FormattedInventoryItem[] }> {
  const targetBranchId = await resolveBranchId(branchId);

  const rows = await query('inventory_items')
    .where({ branch_id: targetBranchId })
    .orderBy('name_ar', 'asc');

  const items: FormattedInventoryItem[] = rows.map((row) => ({
    id: row.id,
...
export async function createInventoryItem(
  ...
  const targetBranchId = await resolveBranchId(input.branch_id || sessionBranchId);
  const itemId = `inv_${crypto.randomUUID()}`;

  const minQty = input.min_qty !== undefined ? input.min_qty : null;

  await query('inventory_items').insert({
    id: itemId,
  ...
  const created = await query('inventory_items').where({ id: itemId }).first();
...
export async function updateInventoryItem(
  ...
  const item = await query('inventory_items').where({ id }).first();
  if (!item) {
    throw new AppError('NOT_FOUND', 'الصنف غير موجود في المخزون', 404);
  }
  ...
  if (input.min_qty !== undefined) updateData.min_qty = input.min_qty;
  if (input.unit !== undefined) updateData.unit = input.unit;
  if (input.is_active !== undefined) updateData.is_active = input.is_active ? 1 : 0;

  await query('inventory_items').where({ id }).update(updateData);

  const updated = await query('inventory_items').where({ id }).first();

  // فحص التنبيه بعد التعديل
  await checkAndCreateLowStockAlert(
    updated.name_ar,
    updated.qty,
    updated.min_qty ?? null,
    updated.branch_id
  );
 ...
 * @throws {AppError} 404 NOT_FOUND في حال عدم وجود الصنف
 */
export async function adjustInventoryQty(
  id: string,
  input: AdjustQtyInput
): Promise<{ id: string; qty: number; min_qty: number | null; low_stock: boolean }> {
  const item = await query('inventory_items').where({ id }).first();
  if (!item) {
    throw new AppError('NOT_FOUND', 'الصنف غير موجود في المخزون', 404);
  }

  await query('inventory_items').where({ id }).update({
    qty: input.qty,
    updated_at: query.raw("(datetime('now'))"),
  });
